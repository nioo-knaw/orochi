...
rule sort_per_treatment:
    input:
        forward=lambda wildcards: expand("scratch/host_filtering/{sample}_R1.fastq", sample=config["data"][wildcards.treatment]) if config['host_removal'] \
             else expand("scratch/filter/{sample}_R1.fasta", sample=config["data"][wildcards.treatment]),
        reverse=lambda wildcards: expand("scratch/host_filtering/{sample}_R2.fastq", sample=config["data"][wildcards.treatment]) if config['host_removal'] \
             else expand("scratch/filter/{sample}_R2.fasta", sample=config["data"][wildcards.treatment]),
    output:
