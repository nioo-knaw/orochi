...
rule sort_per_treatment:
    ...
    output:
        forward =lambda wildcards: expand("scratch/sorted/{treatment}/{sample}_R1.fastq", sample=config["treatment"][wildcards.treatment]),
        reverse =lambda wildcards: expand("scratch/sorted/{treatment}/{sample}_R2.fastq", sample=config["treatment"][wildcards.treatment])
    run: 
        if config['host_removal']:
