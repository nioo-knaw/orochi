...
rule coverage:
    input:
        forward = expand("scratch/coverm/bamfiles/secondary.contigs.fasta.{sample}_R1.fastq.bam", sample=config["data"]),
        reverse = expand("scratch/coverm/bamfiles/secondary.contigs.fasta.{sample}_R2.fastq.bam", sample=config["data"]),
    output:
    ...
    threads: 16
    shell:
        "coverm contig -b {input.forward} {input.reverse} -t {threads} -o {output}"
