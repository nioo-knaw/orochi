...
with open("config.json", "w") as file:
    ...
    for i in range(len(main_name_list)):
        if i == len(main_name_list)-1:
            file.write('    ' + '   ' + "\"" + main_name_list[i] + '" : ["' + main_name_list[i] + '"]' + '\n')
        else:
            file.write('    ' + '   ' + "\"" + main_name_list[i] + '" : ["' + main_name_list[i] + '"],' + '\n')
    #print('    ' + '   ' + '"all" : ' + str(main_name_list) + '\n')
    file.write('     ' + x_1.format("") + "," + '\n')
    file.write('     ' + data_line + x_0.format("") + '\n')


## This is for printing the data (samples)
for i in range(len(main_name_list)):
    with open("config.json", "a") as file:
        if i == len(main_name_list)-1:
            file.write('    ' + '   ' + "\"" + main_name_list[i] + '": {"forward": ' + "\"" + data_path_line + forward_list[i] + "\"" + ', "rev": ' + "\"" + data_path_line + reverse_list[i] + "\"}" + '\n')
        else:
            file.write('    ' + '   ' + "\"" + main_name_list[i] + '": {"forward": ' + "\"" + data_path_line + forward_list[i] + "\"" + ', "rev": ' + "\"" + data_path_line + reverse_list[i] + "\"}," + '\n')
...
with open("config.json", "a") as file:
    file.write('     ' + x_1.format("") + '\n')
    file.write(x_1.format("") + '\n')
    print("Configuration file successfully created. Chosen assembly mode: individual assemblies")
