...
rule supervised_pooling:
    ...
    output:
        forward = "results/03_coassembly/pools/{sample_pool}_forward.fastq",
        rev = "results/03_coassembly/pools/{sample_pool}_rev.fastq",
    log:
...
# rule unsupervised_pooling:
#     input:
#         forward=lambda wildcards: expand("results/02_filtered_reads/{sample}_filt_1.fastq.gz", sample=samples["sample"][wildcards.sample]),
#         rev=lambda wildcards: expand("results/02_filtered_reads/{sample}_filt_2.fastq.gz", sample=samples["sample"][wildcards.sample])
...
if config['assembly_method']=='coassembly':
    ...
    #        unpaired = "scratch/treatment/{treatment}_unpaired.fastq"
        output:
            contigs="results/03_coassembly/{sample_pool}/final.contigs.fa",
        ...
        conda:
            "../envs/megahit.yaml"
        shell:"megahit --out-dir results/03_coassembly/{wildcards.sample_pool}/final.contigs.fa -m 0.9 -t {threads} --presets meta-large -1 {input.forward} -2 {input.rev}"
