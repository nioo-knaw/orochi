...
for (level in taxa_levels) {
  ...
  p2 <- p2 + labs(y = "Relative Abundance (%)", title = paste("(Prokaryotic) Taxonomic Abundance at", level)) +
...
for (level in taxa_levels) {
  ...
  p3 <- p3 + labs(y = "Relative Abundance Normalized by assigned Reads (%)", title = paste("(Prokaryotic) Taxonomic Abundance at", level)) +
    theme(plot.title = element_text(hjust = 0.5))
  
  # Save each plot with its level in filename
  ggplot2::ggsave(
    filename = file.path(outdir, plotsdir,paste0("2-taxonomic_normalized_relative_abundance_", tolower(level), ".tiff")),
    plot = p3,
    width = 12,
    height = 10,
    units = "in",
  ...
  # Save each plot with its level in filename
  ggplot2::ggsave(
    filename = file.path(outdir, plotsdir, paste0("3-taxonomic_normalized_relative_abundance_heatmap_", tolower(level), ".tiff")),
    plot = h2_gg,
    width = 12,
    height = 10,
    units = "in",
...
for (tr in treatments) {
  ...
  # Some genomes may be assigned multiple taxonomies, keep only the first one
  tax <- tax[!duplicated(tax$X..bin), ]
