...
if (opt$diversity == "beta"){
  ...
  if (opt$metric == "bray-curtis"){
    mat <- rbiom::bdiv_distmat(as.matrix(mpa_table), bdiv="Bray-Curtis", weighted=TRUE)
  }
  
  # Jaccard
  if (opt$metric == "jaccard"){
    mat <- rbiom::bdiv_distmat(as.matrix(mpa_table), bdiv="Jaccard", weighted=FALSE)
  }
  ...
  if (grepl("unifrac", opt$metric)){
    ...
    if (opt$metric == "weighted-unifrac"){
      mat <- rbiom::bdiv_distmat(as.matrix(filt_mpa_table), tree=filt_tree, bdiv="UniFrac", weighted=TRUE)
      
    } else if (opt$metric == "unweighted-unifrac"){
      mat <- rbiom::bdiv_distmat(as.matrix(filt_mpa_table), tree=filt_tree, bdiv="UniFrac", weighted=FALSE)
    } 
  ...
  if (opt$metric == "clr" || opt$metric == "aitchison"){
    ...
    if (opt$metric == "aitchison"){
      # Aitchison
      mat <- rbiom::bdiv_distmat(mat, method="euclidean", weighted=TRUE)
    }
  }
